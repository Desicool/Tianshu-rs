...
| Crate | 说明 |
|---|---|
| `workflow_engine` | 核心库：调度器、trait 接口、内存存储 |
| `workflow_engine_postgres` | `CaseStore` + `StateStore` 的 PostgreSQL 实现 |
| `workflow_engine_llm_openai` | 兼容 OpenAI 接口的 `LlmProvider` 适配器 |
...
- [ ] 步骤级耗时 span
- [ ] `workflow_engine_sqlite`——适用于轻量部署的 SQLite 适配器
- [ ] `workflow_engine_mongodb`——MongoDB 适配器
- [ ] `workflow_engine_llm_anthropic`——Claude API 适配器
- [ ] 意图路由示例（基于 LLM 的消息分类）
- [ ] 工作流管理 API（查看与重放）
