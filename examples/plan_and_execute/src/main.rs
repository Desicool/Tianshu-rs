//! Plan-and-Execute workflow CLI runner.
//!
...
//!
//! Usage:
//!   cargo run -p plan_and_execute -- "What are the top 3 largest files in /tmp?"
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let query = if args.is_empty() {
        "Explain the three main phases of the Plan-and-Execute AI agent pattern and \
         give a concrete example of how each phase would work for the task: \
         'Find the top 5 most-edited files in a git repository.'"
            .to_string()
    } else {
        args.join(" ")
    };
    ...
    // ── Case setup ────────────────────────────────────────────────────────────
    let case_key = "plan_and_execute_demo_001";
    let mut case = Case::new(
        case_key.into(),
        "demo_session".into(),
        "plan_and_execute".into(),
    );
    ...
    info!("Starting plan-and-execute workflow...");
    ...
    loop {
        ...
        match result {
            ...
            TickResult::Idle => {
                // No runnable workflows — either all finished or all waiting.
