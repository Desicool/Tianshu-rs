//! ReAct agent workflow CLI runner.
//!
...
//!
//! Usage:
//!   cargo run -p react_agent -- "What files are in /tmp?"
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let task = if args.is_empty() {
        "Using available tools, find out how many Rust source files are in the \
         current directory tree and report the count."
            .to_string()
    } else {
        args.join(" ")
    };
    ...
    register_workflows(&mut registry, Arc::clone(&llm), model.clone(), 10);

    // ── Case setup ────────────────────────────────────────────────────────────
    let case_key = "react_agent_demo_001";
    let mut case = Case::new(case_key.into(), "demo_session".into(), "react_agent".into());
    ...
    info!("Starting ReAct agent workflow...");
